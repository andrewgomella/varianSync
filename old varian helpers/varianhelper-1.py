...
# NIDAQ lines
EXP_OK          = "Dev2/port0/line1"
EXP_REQ         = "Dev2/port0/line0"
...
class myDriver(Driver):
    def  __init__(self):
        ...
        DAQmxCreateDOChan(self.taskHandle2, EXP_REQ, "", DAQmx_Val_ChanForAllLines)
        DAQmxSetDOOutputDriveType(self.taskHandle2, EXP_REQ, DAQmx_Val_ActiveDrive);
    ...
    def pollexpok(self):
        ...
        while True:
            ...
            print oldval,x
            if oldval != x:
                if x == 1 and VARIAN_PV.get(as_string=False) == 1:
                    print "X-ray ON"
                    # Source-ray is being used! 
                    if self.getParam('XSYNC') == 0:
                        self.time1 = time.time()
                        caput(XRAY_IOC + 'ON', '1')
                        self.setParam('EXP_OK', 1)
                        temp = self.time1 - self.time2
                        if temp > 1000000:
                            print "Time since x-ray off ", "IDKLOL", " seconds"
                        else:
                            print "Time since x-ray off ", temp , " seconds"
                    # Oxford is being used!   
                    if self.getParam('XSYNC') == 1:
                        self.time1 = time.time()
                        caput(XRAY_IOC + 'ON', '1')            
                        while caget(XRAY_IOC + 'STATUS_STRING') != 2:
                            time.sleep(0.01)
                        # Oxford kVp and watt values can only be set during output, so read first
                        xkv = int(caget(XRAY_IOC + 'KVP'))
                        xwatt = int(caget(XRAY_IOC + 'WATT'))
                        caput(XRAY_IOC + 'KVP', str(xkv))
                        caput(XRAY_IOC + 'WATT', str(xwatt))          
                        while int(caget(XRAY_IOC + 'WATT_RBV').split("=")[-1]) != xwatt:
                            time.sleep(.01)
                        while int(caget(XRAY_IOC + 'KVP_RBV').split("=")[-1]) != xkv:
                            time.sleep(.01)
                        self.setParam('EXP_OK', 1)
                        temp = self.time1 - self.time2
                        if temp > 1000000:
...
#                    print oldval,x
                    ...
                    if self.getParam('XSYNC') == 0:
                        caput(XRAY_IOC + 'ON', 0)
                    elif self.getParam('XSYNC') == 1:
                        caput(XRAY_IOC + 'OFF', 0)
                    self.setParam('EXP_OK', 0)
                DAQmxReadDigitalLines(self.taskHandle, 1, 1, 0, oldval, 1, None, None, None)
            time.sleep(.001)
